...
export interface RideCheckResult {
  ...
  estimasi_biaya_perbaikan: string; // Tambahkan ini!
...
export async function analyzeVehicleImages(
  ...
  try {
    ...
    // Ask it to parse the images according to the instruction
    ...
    // Use standard gemini-1.5-pro or flash-preview that supports audio natively
    let response;
    try {
    ...
    console.error("Error analyzing images/audio:", error);
    throw error;
  }
}
